...
unsigned char arrayOfNotes[5] = {1,2,3,4,5};
...
void TickSpeaker(){
	...
	switch(speaker){
		case speakerOff:
			if(press == 0x04) speaker = On;
            else if(press == 0x01) speaker = Inc;
            else if(press == 0x02) speaker = Dec;
	...
	switch(speaker){
		...
		case On:
			if(tickSpeaker <= arrayOfNotes[note]) speaker = 0x10;
	        else if(tickSpeaker <= arrayOfNotes[note]*2) speaker = 0x00;
			else tickSpeaker = 0;
			tickSpeaker++;
			break;
